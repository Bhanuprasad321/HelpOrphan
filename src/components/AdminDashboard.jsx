...
const ConfirmationModal = ({ isOpen, type, title, message, onConfirm, onClose }) => {
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                >
                    <h3 className={`text-xl font-bold mb-4 ${isConfirm ? 'text-red-400' : 'text-yellow-400'}`}>
                        {title}
                    </h3>
                    ...
                    <div className="flex justify-end space-x-3">
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-white font-semibold transition duration-150 shadow-md ${
                                isConfirm ? 'bg-red-600 hover:bg-red-500' : 'bg-indigo-600 hover:bg-indigo-500'
                            }`}
                        >
...
export default function AdminDashboard() {
    ...
    const fetchItems = useCallback(async (showLoading = true) => {
        try {
            ...
            setError("Failed to load data from API. Ensure your backend server is running and the endpoint is correct.");
        } finally {
    ...
    const handleDelete = (itemId, itemName) => {
        if (!isAdmin) {
            setModal({
                isOpen: true, type: 'alert', title: 'Access Denied',
                message: 'Only admins can delete items.', onClose: () => setModal({ ...modal, isOpen: false }),
            });
            return;
        }
    ...
    const executeDelete = async (itemId, itemName) => {
        ...
        try {
            ...
            if (!user) {
                setModal({
                    isOpen: true, type: 'alert', title: 'Authentication Error',
                    message: 'Authentication error. Please log in again.', onClose: () => setModal({ ...modal, isOpen: false }),
                });
                return;
            }
            ...
            if (res.ok) {
                ...
                setModal({
                    ...
                    message: `"${itemName}" deleted successfully.`, onClose: () => setModal({ ...modal, isOpen: false }),
                });
            } else {
            ...
            setModal({
                ...
                message: `Deletion failed: ${err.message}. Refreshing data...`, onConfirm: fetchItems,
            });
        }
    };
    ...
    const handleToggleFulfillment = async (item) => {
        if (!isAdmin) {
            setModal({
                isOpen: true, type: 'alert', title: 'Access Denied',
                message: 'Only admins can change fulfillment status.', onClose: () => setModal({ ...modal, isOpen: false }),
            });
            return;
        }
        ...
        setWishlistItems(prev => prev.map(i => 
            (i._id || i.id) === itemId ? { ...i, status: newStatus, fulfilled: newStatus === 'Fulfilled' } : i
        ));
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.message || `Failed to update status. Server status: ${res.status}`);
            }
            ...
            setWishlistItems(prev => prev.map(i => 
                (i._id || i.id) === itemId ? { ...i, status: originalStatus, fulfilled: originalStatus === 'Fulfilled' } : i
            ));
            
            setModal({
                isOpen: true, type: 'alert', title: 'Update Failed',
                message: `Failed to update status: ${err.message}`, onClose: () => setModal({ ...modal, isOpen: false }),
            });
        }
    };
        ...
        >
            ...
            <button 
                ...
                className={`mb-6 flex items-center bg-indigo-600 text-white px-4 py-2 rounded-lg transition duration-150 shadow-md ${
                    isRefreshing ? 'bg-indigo-700 cursor-not-allowed' : 'hover:bg-indigo-500'
                }`}
            >
                {isRefreshing ? (
            ...
            <p className="mb-4 text-sm text-gray-400">
                Logged in as: <span className="font-semibold text-white">{currentUser?.email || 'Guest'}</span>
            </p>

            {!isAdmin && (
                <p className="mb-4 text-orange-400 font-semibold border-l-4 border-orange-400 pl-3 py-1 bg-gray-800 rounded-md">
                    ⚠️ Access: You are not recognized as the administrator. Edit and Delete actions are disabled.
                </p>
            )}
            ...
            <div className="bg-gray-800 p-6 mb-10 rounded-xl shadow-lg border border-gray-700">
                ...
                {totalItems === 0 ? (
                    <p className="text-gray-400 italic">No items have been added to the wishlist yet.</p>
                ) : (
                    <div className="flex flex-col md:flex-row items-center justify-around gap-6">
                        ...
                        <div className="space-y-3">
                            <p className="text-lg">
                                ...
                                Fulfilled: <strong className="text-emerald-400">{fulfilledItems.length}</strong> ({fulfilledPercentage}%)
                            </p>
                            <p className="text-lg">
                                <span className="inline-block w-4 h-4 bg-amber-400 rounded-full mr-2"></span>
                                Pending: <strong className="text-amber-400">{pendingItems.length}</strong> ({pendingPercentage}%)
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div>
                    ...
                    <ul className="space-y-4">
                        {pendingItems.length === 0 ? (
                            <p className="text-gray-500 italic p-4 bg-gray-800 rounded-lg">All items are currently marked as fulfilled!</p>
                        ) : (
                            pendingItems.map(item => (
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap items-baseline gap-x-4">
                                        <span className="text-xl font-medium text-white">{item.name || item.item || 'N/A'}</span>
                                        <span className="text-sm text-gray-400">Qty: {item.quantity || 0}</span>
                                        <span className={`text-xs text-white px-2 py-0.5 rounded-full font-semibold mt-1 ${getUrgencyClass(item.urgency)}`}>
                                    ...
                                    <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-700">
                                        <button
                                            ...
                                            className={`text-sm text-white px-3 py-1 rounded-md transition ${
                                                isAdmin ? 'bg-green-600 hover:bg-green-500' : 'bg-gray-600 cursor-not-allowed'
                                            }`}
                                        >
                                            ✅ Mark Fulfilled
                                        </button>
                                        <button
                                            ...
                                            disabled={!isAdmin}
                                            className={`text-sm text-white px-3 py-1 rounded-md transition ${
                                                isAdmin ? 'bg-blue-600 hover:bg-blue-500' : 'bg-gray-600 cursor-not-allowed'
                                            }`}
                                        >
                                            ✏️ Edit
                                        </button>
                ...
                <div>
                    ...
                    <ul className="space-y-4">
                        {fulfilledItems.length === 0 ? (
                            <p className="text-gray-500 italic p-4 bg-gray-800 rounded-lg">No items have been fulfilled yet.</p>
                        ) : (
                                ...
                                >
                                    <div className="flex flex-wrap items-center justify-between">
                                        <div className="flex flex-col sm:flex-row sm:items-baseline sm:gap-4">
                                            <span className="font-medium text-gray-500 line-through text-lg">{item.name || item.item || 'N/A'}</span>
                                    ...
                                    <div className="flex gap-2 pt-2 border-t border-gray-700">
                                        <button
                                            ...
                                            disabled={!isAdmin}
                                            className={`text-sm text-white px-3 py-1 rounded-md transition ${
                                                isAdmin ? 'bg-yellow-600 hover:bg-yellow-500' : 'bg-gray-600 cursor-not-allowed'
                                            }`}
                                        >
                                            ↩️ Mark Pending
                                        </button>
